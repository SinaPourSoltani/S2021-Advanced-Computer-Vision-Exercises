...
def objective(params, n_cams, n_Qs, cam_idxs, Q_idxs, qs):
    ...
    cam_params = np.reshape(params[:n_cams*9], (n_cams, 9))
    Qs = np.reshape(params[n_cams*9:], (n_Qs, 3))

    qs_proj = project(Qs[Q_idxs], cam_params[cam_idxs])
    residual = qs - qs_proj
    return np.ravel(residual, order='F')
...
def bundle_adjustment(cam_params, Qs, cam_idxs, Q_idxs, qs):
    ...
    params = np.concatenate((cam_params.flatten(), Qs.flatten()), axis=-1)
    residual_init = objective(params, n_cams, n_Qs, cam_idxs, Q_idxs, qs)
    res = least_squares(objective, params, method='trf', ftol=1e-4, x_scale='jac', verbose=2, args=[n_cams, n_Qs, cam_idxs, Q_idxs, qs])
    residual_solu = objective(res.x, n_cams, n_Qs, cam_idxs, Q_idxs, qs)
    return residual_init, residual_solu, res.x
...
def sparsity_matrix(n_cams, n_Qs, cam_idxs, Q_idxs):
    ...
    for obs in range(cam_idxs.size):
        # set camera
        sparse_mat[2 * obs, 9 * cam_idxs[obs]: 9 * cam_idxs[obs] + 9] = 1
        sparse_mat[2 * obs + 1, 9 * cam_idxs[obs]: 9 * cam_idxs[obs] + 9] = 1

        # set Q points
        sparse_mat[2 * obs, 9 * n_cams + 3 * Q_idxs[obs]:9 * n_cams + 3 * Q_idxs[obs] + 3] = 1
        sparse_mat[2 * obs + 1, 9 * n_cams + 3 * Q_idxs[obs]:9 * n_cams + 3 * Q_idxs[obs] + 3] = 1
...
def bundle_adjustment_with_sparsity(cam_params, Qs, cam_idxs, Q_idxs, qs, sparse_mat):
    ...
    return residual_init, residual_solu, solu
