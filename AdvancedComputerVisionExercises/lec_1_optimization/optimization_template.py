...
def define_stereo_cameras():
    ...
    return StereoCameras(400, 200, 1, D, t_r, r_r)
...
def project_points_to_image_plane(Qs, cam):
    ...
    qs = cv2.projectPoints(Qs, cam.r, cam.t, cam.K, cam.D)[0]
    #print(np.squeeze(qs).T
    return np.squeeze(qs).T
...
def add_noise_to_image_points(qs, sigma):
    ...
    qs_noisy = qs + np.random.normal(0, sigma, qs.shape)
    #print(qs_noisy)
...
def triangulate_points(qs_l, qs_r, cams):
    ...
    M = cv2.triangulatePoints(cams.cam_l.P, cams.cam_r.P, qs_l, qs_r)
    uhom_Qs = M / M[-1]
    return uhom_Qs[0:3].T
...
def evaluate_reprojection_error(Qs, qs_l, qs_r, cams):
    ...
    rqs_l = project_points_to_image_plane(Qs, cams.cam_l)
    rqs_r = project_points_to_image_plane(Qs, cams.cam_r)

    rms_l = np.sqrt(((rqs_l - qs_l) * (rqs_l - qs_l)) / len(rqs_l))
    rms_r = np.sqrt(((rqs_r - qs_r) * (rqs_r - qs_r) / len(rqs_r)))
    rms_total = rms_l + rms_r

    return sum(sum(rms_total))
...
def optimize_points(Qs, qs_l, qs_r, cams):
    ...
    for q_l, q_r, Q_init in zip(qs_l, qs_r, Qs):
        ...
        params.add('x', Q_init[0])
        params.add('y', Q_init[1])
        params.add('z', Q_init[2])
        fit = lmfit.minimize(objective_func, params, args=(q_l, q_r, cams.cam_l.P, cams.cam_r.P))
